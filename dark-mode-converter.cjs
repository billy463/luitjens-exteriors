...
function processDir(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      // Clean up backgrounds to Dark Mode
      ...
      content = content.replace(/text-dark(?!er)/g, 'text-gray-200');
      
      // Ring & Border Inversions
...
console.log('Dark mode conversion CJS script completed successfully.');
